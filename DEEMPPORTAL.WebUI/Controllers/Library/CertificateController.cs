  ...
  ICertificateService certificateService) : BaseController
{
  ...
  {
    var results = await _certificateService.GetAllLibraryInformation(orgCode);
  ...
  {
    var results = await _certificateService.GetLibraryInformation(libraryInformationCode);
  ...
  public async Task<IActionResult> UpdSertLibraryInformation([FromForm] LibraryInformationDetailViewModel model)
  {
    ...
    if (!await _certificateService.SaveLibraryInformation(mapped))
    ...
    {
      isSuccess = true,
      message = "Certificate library saved successfully."
    });
  }
  ...
  {
    if (!await _certificateService.DeleteLibraryInformation(libraryInformationCode))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    var results = await _certificateService.GetAllLibraryAttchment(libraryInformationCode);
  ...
  {
    var results = await _certificateService.GetLibraryAttachment(libraryInformationCode);
  ...
  public async Task<IActionResult> InsertLibraryAttachments([FromForm] LibraryAttachmentViewModel model)
  {
    ...
    if (!await _certificateService.InsertLibraryAttachment(model.LIBRARY_INFORMATION_CODE, model.FILE_ATTACHMENT))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    if (!await _certificateService.DeleteLibraryAttachment(libraryAttachmentCode))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    var file = await _certificateService.GetLibraryAttachment(libraryAttachmentCode);
