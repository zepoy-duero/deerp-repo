...
public class LogoController(
  ...
  ILogoService logoService) : BaseController
{
  private readonly ISelectOptionsService _selectOptionsService = selectOptionsService;
  private readonly ILogoService _logoService = logoService;
  ...
  {
    var results = await _logoService.GetAllLibraryInformation(orgCode);
  ...
  {
    var results = await _logoService.GetLibraryInformation(libraryInformationCode);
  ...
  public async Task<IActionResult> UpdSertLibraryInformation([FromForm] LibraryInformationDetailViewModel model)
  {
    ...
    if (!await _logoService.SaveLibraryInformation(mapped))
  ...
  {
    if (!await _logoService.DeleteLibraryInformation(libraryInformationCode))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    var results = await _logoService.GetAllLibraryAttchment(libraryInformationCode);
  ...
  {
    var results = await _logoService.GetLibraryAttachment(libraryInformationCode);
  ...
  public async Task<IActionResult> InsertLibraryAttachments([FromForm] LibraryAttachmentViewModel model)
  {
    ...
    if (!await _logoService.InsertLibraryAttachment(model.LIBRARY_INFORMATION_CODE, model.FILE_ATTACHMENT))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    if (!await _logoService.DeleteLibraryAttachment(libraryAttachmentCode))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    var file = await _logoService.GetLibraryAttachment(libraryAttachmentCode);
