...
[Authorize]
[Route("library/credit-application")]
public class CreditApplicationController(
  IMapper mapper,
  ISelectOptionsService selectOptionsService,
  ICreditApplicationService creditApplicationService) : BaseController
{
  private readonly ISelectOptionsService _selectOptionsService = selectOptionsService;
  private readonly ICreditApplicationService _creditApplicationService = creditApplicationService;
  ...
  {
    var results = await _creditApplicationService.GetAllLibraryInformation(orgCode);
  ...
  {
    var results = await _creditApplicationService.GetLibraryInformation(libraryInformationCode);
  ...
  public async Task<IActionResult> UpdSertLibraryInformation([FromForm] LibraryInformationDetailViewModel model)
  {
    ...
    if (!await _creditApplicationService.SaveLibraryInformation(mapped))
    ...
    {
      isSuccess = true,
      message = "Logo library saved successfully."
    });
  }
  ...
  {
    if (!await _creditApplicationService.DeleteLibraryInformation(libraryInformationCode))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    var results = await _creditApplicationService.GetAllLibraryAttchment(libraryInformationCode);
  ...
  {
    var results = await _creditApplicationService.GetLibraryAttachment(libraryInformationCode);
  ...
  public async Task<IActionResult> InsertLibraryAttachments([FromForm] LibraryAttachmentViewModel model)
  {
    ...
    if (!await _creditApplicationService.InsertLibraryAttachment(model.LIBRARY_INFORMATION_CODE, model.FILE_ATTACHMENT))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    if (!await _creditApplicationService.DeleteLibraryAttachment(libraryAttachmentCode))
      return BadRequest(new
      {
        isSuccess = false,
  ...
  {
    var file = await _creditApplicationService.GetLibraryAttachment(libraryAttachmentCode);
