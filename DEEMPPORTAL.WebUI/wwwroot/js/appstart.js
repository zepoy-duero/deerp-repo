...
async function getAllMenuItems() {
  try {
    const response = await fetch(`/main/getMainMenus`);
    const data = await response.json()
    const totalCount = data.length;
    let li = ""

    if (totalCount > 0) {
      const menuItems = await Promise.all(data.map(async (item) => {
        if (item.HAS_SUB_MENU === 'N') {
          ...
          return `
						<li class="nav-item">
							<a href="javascript:void(0)" class="sidenav-menu" data-bs-toggle="collapse" has-submenu="${item.HAS_SUB_MENU}" mm-code="${item.MAIN_MENU_CODE}" onclick="toggleSideSubMenuItems(this)">
								<i class="${item.ICON_NAME}"></i>
								<p>${item.MENU_NAME}</p>
								<span class="caret"></span>
							</a>
							<div class="collapse ps-1" id="menu-code-${item.MAIN_MENU_CODE}-0-0"></div>
						</li>
					`
        }
      }))
    ...
    $(".sidebar-wrapper .sidebar-content").empty().append(ul)
...
async function toggleSideSubMenuItems(element) {
  const mainMenuCode = $(element).attr("mm-code")
  const subMenuItems = await getAllSubMenuItems(mainMenuCode)

  let $subMenuDiv = $(`#menu-code-${mainMenuCode}-0-0`)

  if ($subMenuDiv.is(":empty")) {
    $subMenuDiv.empty().append(subMenuItems)
  }

  $subMenuDiv.collapse("toggle")
}

async function getAllSubMenuItems(mainMenuCode) {
  try {
    const response = await fetch(`/main/getSubMenus?mainMenuCode=${mainMenuCode}`);
    const data = await response.json()
    const totalCount = data.length;
    let li = ""
      ...
      const menuItems = await Promise.all(data.map(async (item) => {
        if (item.HAS_SUB_MENU === 'N') {
          return `
						<li>
							<a class="sidenav-menu" href="/page?name=${item.MENU_NAME}" has-submenu="${item.HAS_SUB_MENU}">
								<span class="sub-item">${item.MENU_NAME}</span>
							</a>
						</li>
					`
        } else {
          return `
						<li>
							<a href="javascript:void(0)" class="sidenav-menu" data-bs-toggle="collapse" has-submenu="${item.HAS_SUB_MENU}" mm-code="${item.MAIN_MENU_CODE}" sm-code="${item.MENU_SUB_CODE}" onclick="toggleSideSubLevelMenuItems(this)">
								<span class="sub-item">${item.MENU_NAME}</span>
								<span class="caret"></span>
							</a>
							<div class="collapse ps-3" id="menu-code-${item.MAIN_MENU_CODE}-${item.MENU_SUB_CODE}-0"></div>
						</li>
					`
        }
      }))
...
async function toggleSideSubLevelMenuItems(element) {
  const mainMenuCode = $(element).attr("mm-code")
  const subMenuCode = $(element).attr("sm-code")

  // Find the corresponding div 
  const $subLevelMenuDiv = $(`#menu-code-${mainMenuCode}-${subMenuCode}-0`)

  // Fetch the sub level menu items
  const menuItems = await getAllSubLevelMenuItems(mainMenuCode, subMenuCode)

  // Check if content is already populated
  if ($subLevelMenuDiv.is(":empty")) {
    // Dynamic content
    $subLevelMenuDiv
      .empty()
      .append(menuItems)
  ...
  $subLevelMenuDiv.collapse("toggle")
}

async function getAllSubLevelMenuItems(mainMenuCode, subMenuCode) {
  try {
    const response = await fetch(`/main/getSubLevelMenus?mainMenuCode=${mainMenuCode}&subMenuCode=${subMenuCode}`);
    const data = await response.json()
    const totalCount = data.length;
    let li = ""
    ...
    if (totalCount > 0) {
      const menuItems = await Promise.all(data.map(async (item) => {
        return `
				<li>
					<a class="sidenav-menu" href="/page/${removeUrlSpecialCharacters(item.MENU_URL)}" has-submenu="${item.HAS_SUB_MENU}">
						<span class="sub-item">${item.MENU_NAME}</span>
					</a>
				</li>
			`
