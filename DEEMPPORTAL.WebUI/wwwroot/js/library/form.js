...
function showModalCreateNewFormInformation() {
    $(gModal).find(".modal-title").text("Create New Form Entry")
    $(gModal).find("#inpLibraryTypeCode").val("FORM")
    $(gModal).modal("show")

    gIsUpdate = false
}

async function showModalEditFormInformation(libraryInformationCode) {
    $(gModal).find(".modal-title").text("Edit Form Information")
    $(gModal).modal("show")
    ...
    $(gFormLibraryInformation).find("#inpLibraryTypeCode").val("FORM")
}
...
async function displayLibraryInformation() {
    ...
    if (totalCount > 0) {
        for (let i = 0; i < totalCount; i++) {
            if (userId === '1') {
                btnActions = `
          <a href="javascript:void(0)" class="text-muted mx-1" onclick="showModalEditFormInformation(${data[i].LIBRARY_INFORMATION_CODE})">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
          </a>
          <a href="javascript:void(0)" class="text-danger mx-1" onclick="deleteFormInformation(${data[i].LIBRARY_INFORMATION_CODE})">
...
async function deleteFormInformation(libraryInformationCode) {
