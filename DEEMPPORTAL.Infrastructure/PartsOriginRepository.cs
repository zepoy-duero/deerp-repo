...
public List<SelectListItem> GetOrganizations()
        ...
        string supCode)
    {
        ...
        {
            using var cmd = new SqlCommand("CLOUD_v1_ERP_PARTS_ORIGIN_HSCODE_insrt_2", con);
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            cmd.Parameters.Add("@CTGY_CODE", SqlDbType.Int).Value = r.CTGY_CODE;
            cmd.Parameters.AddWithValue("@DESCRIPTION", (object?)r.Description ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@WEIGHT", (object?)r.Weight ?? DBNull.Value);
    ...
    public List<SingleRecordUpdateModel.OriginRow> SearchOriginHsCode(int ctgyCode, string supplierCode, string partNo)
    {
        ...
        using (var con = new SqlConnection(_cp.ConnectionName))
        using (var cmd = new SqlCommand("CLOUD_v1_ERP_PARTS_ORIGIN_SINGLE_RECORD_updt_1", con))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            using (var dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    var rawWeight = dr["WEIGHT"] == DBNull.Value? "" : Convert.ToString(dr["WEIGHT"]) ?? "";
                    list.Add(new SingleRecordUpdateModel.OriginRow
                    {
                        ...
                        CtgyCode = dr["CTGY_CODE"] == DBNull.Value ? 0 : Convert.ToInt32(dr["CTGY_CODE"]),
    ...
    public int UpdateOriginHsCode(SingleRecordUpdateModel.SaveOriginRequest req)
    {
        ...
        cmd.Parameters.AddWithValue("@NEW_WEIGHT", req.Weight ?? "");
    ...
    public List<SelectListItem> GetCategories()
    {
        ...
        using var con = new SqlConnection(_cp.ConnectionName);
        using var cmd = new SqlCommand("CLOUD_v1_ERP_PARTS_ORIGIN_CATEGORY_list_1", con);
        cmd.CommandType = CommandType.StoredProcedure;
    ...
    public PartsOrgViewModel.PartNoResolveResult ResolveBlumaqPartNo(int orgCode, string excelPartNo)
    {
        excelPartNo = (excelPartNo ?? "").Trim();
        if (excelPartNo.Length == 0)
            return new PartsOrgViewModel.PartNoResolveResult { ResolvedPartNo = "", MatchSource = "EMPTY" };

        using var con = new SqlConnection(_cp.ConnectionName); // ✅ change if not a real connection string
        using var cmd = new SqlCommand("CLOUD_v1_ERP_PARTS_ORIGIN_BlumaqPartNo_get_3", con);
        cmd.CommandType = CommandType.StoredProcedure;

        cmd.Parameters.Add("@ORG_CODE", SqlDbType.Int).Value = orgCode;
        cmd.Parameters.Add("@EXCEL_PARTNO", SqlDbType.VarChar, 100).Value = excelPartNo;
        ...
        if (dr.Read())
        {
            return new PartsOrgViewModel.PartNoResolveResult
            {
                ResolvedPartNo = (dr["ResolvedPartNo"]?.ToString() ?? "").Trim(),
                MatchSource = (dr["MatchSource"]?.ToString() ?? "").Trim()
            };
        }

        return new PartsOrgViewModel.PartNoResolveResult { ResolvedPartNo = excelPartNo, MatchSource = "NO_RESULT" };
    }
